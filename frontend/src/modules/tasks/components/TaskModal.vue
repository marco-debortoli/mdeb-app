...
import NotesPanel from "@/shared/components/NotesPanel.vue";
...
// ── Mobile tab state ──────────────────────────────────────────────────────────
const activeTab = ref<"form" | "notes">("form");

const isEdit = computed(() => !!props.task);
const modalTitle = computed(() => (isEdit.value ? "Edit Task" : "New Task"));
const modalMaxWidth = computed(() => (isEdit.value ? "sm:max-w-2xl" : "sm:max-w-lg"));
...
<template>
  ...
  >
    <template #body>
      <!-- Mobile tab bar (edit mode only) -->
      <div v-if="isEdit" class="sm:hidden flex shrink-0 border-b border-parchment-300">
        <button
        ...
        >
          Notes
          <span v-if="fullTask?.notes.length" class="ml-1 text-xs text-slate-400"
            >({{ fullTask.notes.length }})</span
          >
        </button>
      </div>

      <!-- Content area: two-column on desktop, tab-driven on mobile -->
      <div class="flex flex-1 min-h-0">
        <!-- Form panel -->
        <div
          class="flex-col flex-1 overflow-y-auto px-5 py-4 space-y-4"
          :class="[activeTab === 'form' ? 'flex' : 'hidden', 'sm:flex']"
        >
          <!-- Error -->
        ...
        <!-- Notes panel (edit mode only) -->
        <div
          v-if="isEdit"
          class="flex-col border-parchment-300 min-h-0 overflow-hidden sm:w-64 sm:border-l"
          :class="[activeTab === 'notes' ? 'flex flex-1' : 'hidden', 'sm:flex']"
        >
          <NotesPanel
            v-model="newNote"
      ...
      <!-- Footer -->
      <div
        class="flex items-center justify-between px-5 py-4 border-t border-parchment-300 shrink-0"
      >
        <button
          v-if="isEdit"
          :disabled="deleting"
