      ...
      <div class="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        <template v-else-if="profile">
          ...
          <section class="bg-white rounded-lg border border-parchment-200 overflow-hidden">
            ...
            <div v-if="profile.health_log" class="grid grid-cols-2 sm:grid-cols-3 gap-px bg-parchment-100">
              <!-- Steps -->
              <div v-if="profile.health_log.steps != null" class="bg-white px-4 py-2.5">
                <p class="text-xs text-parchment-500">Steps</p>
                <p class="text-sm font-medium text-slate-800">{{ profile.health_log.steps.toLocaleString() }}</p>
              </div>
              <!-- Sleep -->
              <div v-if="profile.health_log.sleep_score != null" class="bg-white px-4 py-2.5">
                <p class="text-xs text-parchment-500">Sleep Score</p>
                <div class="flex items-center gap-1.5">
                  <p class="text-sm font-medium text-slate-800">{{ profile.health_log.sleep_score }}</p>
                  <div v-if="profile.health_log.sleep_rating" class="flex gap-0.5">
                    <span
                      v-for="n in 5"
                      :key="n"
                      class="text-xs leading-none"
                      :class="n <= profile.health_log.sleep_rating! ? 'text-earth-400' : 'text-parchment-300'"
                    >★</span>
                  </div>
                </div>
              </div>
              <!-- Resting HR -->
              <div v-if="profile.health_log.resting_hr != null" class="bg-white px-4 py-2.5">
                <p class="text-xs text-parchment-500">Resting HR</p>
                <p class="text-sm font-medium text-slate-800">
                  {{ profile.health_log.resting_hr }} <span class="text-xs font-normal text-parchment-400">bpm</span>
                </p>
              </div>
              <!-- Body Battery -->
              <div
                v-if="profile.health_log.garmin_body_battery_low != null || profile.health_log.garmin_body_battery_high != null"
                class="bg-white px-4 py-2.5"
              >
                <p class="text-xs text-parchment-500">Body Battery</p>
                <p class="text-sm font-medium text-slate-800">
                  {{ profile.health_log.garmin_body_battery_low ?? '—' }} → {{ profile.health_log.garmin_body_battery_high ?? '—' }}
                </p>
              </div>
              <!-- Stress -->
              <div v-if="profile.health_log.stress_score != null" class="bg-white px-4 py-2.5">
                <p class="text-xs text-parchment-500">Stress</p>
                <p class="text-sm font-medium text-slate-800">{{ profile.health_log.stress_score }}</p>
              </div>
              <!-- Intensity Minutes -->
              <div
                v-if="profile.health_log.intensity_minutes_moderate != null || profile.health_log.intensity_minutes_vigorous != null"
                class="bg-white px-4 py-2.5"
              >
                <p class="text-xs text-parchment-500">Intensity Min</p>
                <p class="text-sm font-medium text-slate-800">
                  {{ profile.health_log.intensity_minutes_moderate ?? '—' }} / {{ profile.health_log.intensity_minutes_vigorous ?? '—' }}
                  <span class="text-xs font-normal text-parchment-400">mod/vig</span>
                </p>
              </div>
              <!-- Energy Rating -->
              <div v-if="profile.health_log.energy_rating != null" class="bg-white px-4 py-2.5">
                ...
                <div class="flex gap-0.5 mt-0.5">
                  <span
                    ...
                    :class="n <= profile.health_log.energy_rating! ? 'text-earth-400' : 'text-parchment-300'"
                  >★</span>
                </div>
              </div>
              <!-- Weight -->
              <div v-if="profile.health_log.weight_kg != null" class="bg-white px-4 py-2.5">
                <p class="text-xs text-parchment-500">Weight</p>
                <p class="text-sm font-medium text-slate-800">
                  {{ Math.round((profile.health_log.weight_kg! / 0.45359237) * 10) / 10 }}
